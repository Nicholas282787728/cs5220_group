 ...
 * 
 * @c*/
void normalize_mass(sim_state_t* s, proc_info* pInfo, sim_param_t* param, int hist[8][4096], int vec_count[8])
{
  if (pInfo->proc == 0) 
  {
  ...
  hash_particles_parallel(s, pInfo, param->h, hist, vec_count);
...
int main(int argc, char** argv)
{
  ...
  int hist[8][HASH_SIZE];


  memset(hist, -1, (8)*HASH_SIZE*sizeof(hist[0][0]) );
  
//  particle_t*** hash_l = globalState->hash_l;
  
  globalState->hash_l  = (particle_t***) calloc(8, sizeof(particle_t**));
  
  for (int i = 0;i<8;i++)
  {
  globalState->hash_l[i] = (particle_t**) calloc(HASH_SIZE, sizeof(particle_t*));
#pragma omp for
  for (int j = 0;j<HASH_SIZE;j++)
  {
  globalState->hash_l[i][j] = NULL;
 // printf("i %d j %d size %d \n", i, j,sizeof(hash_l[i][j]));
  }
  }
  ...
  int vec_count[8];
  memset(vec_count, 0, (8)*sizeof(int) );

#pragma omp parallel shared(globalState, params, hist, vec_count) 
  {
    ...
    normalize_mass(globalState, pInfo, &params, hist, vec_count);
    ...
    hash_particles_parallel(globalState, pInfo, params.h, hist, vec_count);
